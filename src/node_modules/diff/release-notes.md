...
* [#344](https://github.com/kpdecker/jsdiff/issues/344) `diffLines`, `createTwoFilesPatch`, and other patch-creation methods now take an optional `stripTrailingCr: true` option which causes Windows-style `\r` line endings to be replaced with Unix-style  line endings before calculating the diff, just like GNU `diff`'s `--strip-trailing-cr` flag.
